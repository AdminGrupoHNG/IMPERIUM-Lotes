...
using System.Xml;
using static UI_GestionLotes.Formularios.Sistema.Sistema.frmAcercaSistema;
    ...
    public partial class frmLogin : Form
    {
        ...
        private void FormatearClave()
        {
            ...
            if (MessageBox.Show("Se enviará un correo con su nueva clave, ¿Desea continuar?", "Recuperación de clave", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                Program.Sesion.Usuario = unit.Usuario.ObtenerUsuario<eUsuario>(7, txtUsuario.Text);
                if (Program.Sesion.Usuario != null)
                {
                    string sCorreo = Program.Sesion.Usuario.dsc_correo;
                    string sClaveTemp = "Tu nueva contraseña es : " + Program.Sesion.Usuario.dsc_clave;
        ...
        private void IniciarSesion(string Usuario, string Password)
        {
            ...
            Task.Factory.StartNew<eUsuario>(() => {
                ...
                Program.Sesion.Usuario = unit.Usuario.ObtenerUsuario<eUsuario>(2, Usuario);
                return Program.Sesion.Usuario;
            }).ContinueWith((result) => {
                ...
                if (Program.Sesion.Usuario.flg_activo == "NO") { MessageBox.Show("El usuario que ha ingresado se encuentra inactivo en la base de datos", "Permiso denegado!", MessageBoxButtons.OK, MessageBoxIcon.Error); return; }
                if (Program.Sesion.Usuario.flg_noexpira == "NO" && DateTime.Now >= Program.Sesion.Usuario.fch_cambioclave)
                {
                    ...
                    frm.user = Program.Sesion.Usuario;
                    frm.ShowDialog();
                ...
                if (result.Status == TaskStatus.RanToCompletion)
                {
                    if (result.Result.dsc_clave.ToUpper() == Password.ToUpper())
                    {
                        ...
                        if (Program.Sesion.Usuario.flg_cambiar_clave != null) { 
                            if (Program.Sesion.Usuario.flg_cambiar_clave == "SI") {
                                ...
                                //
                                //
                                //frm.Program.Sesion.Usuario.flg_cambiar_clave = Program.Sesion.Usuario.flg_cambiar_clave;
                                frm.ShowDialog();
                                this.Close();
                            } 
                        }
